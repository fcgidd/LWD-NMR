...
{
    //Change2:主控：一直保持查询，回复帧里告诉主控一直为Operation的状态
    //       :事件：回复帧里一直告诉主控Operation的状态，只有当采集完成时，将自行变为IDLE状态
    //       :缺点：回复帧只有IDLE_STAT和OPERATION_STAT，不存在ACQ_FIN_STAT。
    ReplyStateFrame(EventBoardState);
...
void ModeConfirmDeal()
{
    //Change1:主控：一直保持查询，没有采集完成不会收到此回复帧
    //       :事件：等到采集完成，回复帧里告诉主控采集完成，并自行转换为IDLE标志
    //       :缺点：回复帧可以收到ACQ_FIN_STAT状态，但是否影响向主控板重发数据的过程？运行两次的
    if (EventBoardState == ACQ_FIN_STAT)
    {
        ReplyStateFrame(EventBoardState);
    }
